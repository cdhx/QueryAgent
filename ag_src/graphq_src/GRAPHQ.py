...
from graphq_src.wikienv_graphq import WikiEnv
...
def webthink(data_item):
    ...
    for i in range(1, 9):
        ...
        try:
            ...
            if config['self_correction']:
                ...
                action = llm(prompt + f"Thought {i}: {thought}\nAction {i}:", model_name=config['model'],
                             stop=[f"\n"]).strip()
        ...
        this_history = {'step': i, 'Thought': thought, 'Action': action, 'Observation': obs,
                        'Refine': info.get('Refine'), 'current_pyql': env.func_list,
                        'current_sparql': env.pyql.sparql, 'good_call': good_call}
    ...
    solve_history.update(
        {'n_calls': n_calls, 'n_badcalls': n_badcalls, 'prompt': prompt, 'input_token': input_token_cnt,
         'output_token': output_token_cnt, 'cost': this_cost, 'query_cnt': config['query_cnt']})
...
def graphq_main():
    ...
    if config['golden_el'] == False:
        ...
        el_result = {k: v for k, v in el_result.items()}


    for index, item in tqdm(enumerate(json_list), desc='Get linking result from cache'):
        if config['golden_el'] == True:
            json_list[index]['entity_list'] = {x['friendly_name'].strip(): x['id'] for x in item['graph_query']['nodes']
                                               if x['node_type'] == 'entity'}
        else:
            json_list[index]['entity_list'] = {k.strip(): v[0] for k, v in
                                               [v for k, v in el_result.items() if k == str(item['qid'])][0].items()}
    ...
    for data_item in json_list[:100]:
        ...
        total_cost = sum([x['cost'] for x in solve_history_list + [solve_history]]) / len(
            solve_history_list + [solve_history])
        total_query_cnt = sum([x['query_cnt'] for x in solve_history_list + [solve_history]]) / len(
            solve_history_list + [solve_history])
        print('This f1: ', solve_history['f1'], 'this input token: ', solve_history['input_token'],
              'this output token: ', solve_history['output_token'], 'this cost: $', solve_history['cost'],
              'this time: ', solve_history['this_time'], 's, this query cnt: ', solve_history['query_cnt'])
        print('count:', len(rs), ', f1: ', total_f1, ', em: ', total_em, ', time: ', solve_history['time'],
              's, cost: $', total_cost, ', query_cnt: ', total_query_cnt)
        print('-----------\n')
