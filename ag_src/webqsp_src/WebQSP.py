...
from webqsp_src.wikienv_webqsp import WikiEnv
    ...
    for i in range(1, 8):
        ...
        if num_tokens_from_string(prompt)>4090:
            ...
            solve_history.update(
                {'error_info': PROMPT_TOO_LONG_ERROR, 'f1': 0, 'prompt': prompt, 'input_token': input_token_cnt,
        ...
        try:
            ...
            thought, action = re.split(r'\nAction \d: ', thought_action.strip(), maxsplit=1)
...
def webqsp_main():
    ...
    for index, item in tqdm(enumerate(json_list), desc='Get linking result from cache'):
        if config['golden_el'] == True:
            json_list[index]['entity_list'] = item['entity_linking']
        else:
